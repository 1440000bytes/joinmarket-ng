...
- **Neutrino e2e test: subprocess timeout too tight**: `test_reference_taker_coinjoin_with_neutrino_maker_present` inherited `COINJOIN_TIMEOUT = 600s` from the reference coinjoin test. With the neutrino maker present, the reference taker retries counterparty selection, adding significant overhead beyond 600s. Introduced a local `NEUTRINO_COINJOIN_TIMEOUT = 780` (13 minutes) used only for the subprocess call in this test.
