...
test.describe("CoinJoin History Classification", () => {
  ...
  test("address statuses reflect CJ history after direct send", async ({
    ...
    // Get an address to receive in mixdepth 2.
    const { address: destAddr } = await walletApi.getNewAddress(token, 2);

    // Send to ourselves (jar 1 -> jar 2) to create change.
    // Use 40,000 sats to stay comfortably within the available mixdepth 1 balance.
    const sendResult = await walletApi.directSend(token, 1, destAddr, 40_000);
    ...
    // Mixdepth 2 should have a deposit entry.
    const md2 = display.walletinfo.accounts.find((a) => a.account === "2");
    expect(md2).toBeTruthy();
    if (md2) {
      const balance = parseFloat(md2.account_balance);
      expect(balance).toBeGreaterThan(0);
    }

    // Mixdepth 1 should have change (internal branch).
    const md1 = display.walletinfo.accounts.find((a) => a.account === "1");
    expect(md1).toBeTruthy();
    if (md1) {
      // Internal branch entries.
      const internal = md1.branches.find((b) => b.branch.includes("internal"));
      if (internal) {
